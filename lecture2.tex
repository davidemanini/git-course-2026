...
\begin{frame}[fragile]{Note on \LaTeX}
  ...
  When writing a \LaTeX document, keep each sentence in different
  lines.
  \vfill
  {\color{red} Bad:}
\begin{lstlisting}
A vector is called future pointing if $g(v,X)>0$.  Past pointing otherwise.
\end{lstlisting}
  {\color{blue} Good:}
\begin{lstlisting}
A vector is called future pointing if $g(v,X)>0$.
...
\begin{frame}{What GitHub gives you}
  \begin{itemize}
    ...
    \item Collaboration tools: issues, pull requests, reviews
  \end{itemize}
  \vfill
...
\begin{frame}{Why SSH?}
  ...
  \begin{itemize}
    ...
    \item Download GitHub's public keys
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Download GitHub's public keys}
  Copy GitHub's public keys from

%  \url{https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/githubs-ssh-key-fingerprints}

  Paste the content to \texttt{\textasciitilde/.ssh/known\_hosts}
\begin{lstlisting}
cat >> ~/.ssh/known_hosts      # after pasting type Ctrl-D
\end{lstlisting}
...
\end{lstlisting}
  \vfill
  \texttt{origin} is just a nickname for the remote; you can have several.
\end{frame}
...
\begin{frame}{A typical collaborative workflow}
  \begin{enumerate}
    \item \texttt{git clone} the repository
